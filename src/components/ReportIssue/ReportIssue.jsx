...
const ReportIssue = () => {
  ...
  const validateForm = () => {
    ...
    if (!formData.issue.trim()) errors.issue = "Issue description is required";
  ...
  const handleSubmit = (e) => {
    ...
    axiosPublic
      .post("/submit/report-issue", formData) // Assume this is your issue reporting API endpoint
      .then(() => {
        Swal.fire({
          icon: "success",
          title: "Issue Reported",
          text: "Your issue has been reported successfully. We'll look into it.",
        });
      ...
      .catch((error) => {
        Swal.fire({
          icon: "error",
          title: "Error",
          text: "Failed to report issue. Please try again later.",
        });
      });
  };
  ...
  return (
    <div className="py-20 px-5 max-w-3xl mx-auto">
      ...
      <form onSubmit={handleSubmit} className="bg-white p-6 rounded-lg shadow-xl">
        ...
        <div className="mb-4">
          <label className="block text-lg font-medium text-[#008575]">Issue</label>
          <textarea
            name="issue"
